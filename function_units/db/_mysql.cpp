...
int connection::connect_to_db(char *conn_str, char *charset, unsigned int auto_commit_opt)
{
  ...
  m_auto_commit_opt=auto_commit_opt;

  if ( mysql_autocommit(m_conn, m_auto_commit_opt ) != 0 )
  {
    m_cda.rc=mysql_errno(m_conn); strncpy(m_cda.message,mysql_error(m_conn),2000); mysql_close(m_conn); m_conn=NULL;  return -1;
  }
...
int SqlStatement::connect(connection *conn)
{
  ...
  m_auto_commit_opt=m_conn->m_auto_commit_opt;
...
int SqlStatement::prepare(const char *fmt, ...)
{
  ...
  {
    err_report(); memcpy(&m_cda1,&m_cda,sizeof(struct CdaDef)); return m_cda.rc;
  }

  // 判断是否是查询语句，如果是，把m_sql_type设为0，其它语句设为1。
  m_sql_type=1;
  ...
  if (strncmp(strtemp,"SELECT",6)==0) m_sql_type=0;
...
int SqlStatement::bind_in(unsigned int position, int *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
  ...
  if ( (position<1) || (position>=MAX_PARAMS) || (position>m_handle->param_count) )
  {
  ...
  if (position>max_bind_in) max_bind_in=position;

  return 0;
}

int SqlStatement::bind_in(unsigned int position, long *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_in(unsigned int position, unsigned int *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_in(unsigned int position, unsigned long *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_in(unsigned int position, char *value, unsigned int len)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
  ...
  params_in[position-1].is_null= reinterpret_cast<bool *>(&params_in_is_null[position - 1]);
...
int SqlStatement::bind_in_blob(unsigned int position, void *buffer, unsigned long *size)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_in(unsigned int position, float *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_in(unsigned int position, double *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
///////////////////
int SqlStatement::bind_out(unsigned int position, int *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
  ...
  if ( (position<1) || (position>=MAX_PARAMS) || (position>m_handle->field_count) )
  {
    m_cda.rc=-1; strncpy(m_cda.message,"array bound.",128);
  }
...
int SqlStatement::bind_out(unsigned int position, long *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_out(unsigned int position, unsigned int *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_out(unsigned int position, unsigned long *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_out(unsigned int position, char *value, unsigned int len)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_out_blob(unsigned int position, void *buffer, unsigned long buffer_size, unsigned long *size)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
  ...
  params_out[position-1].buffer_length = buffer_size;

  return 0;
}


int SqlStatement::bind_out(unsigned int position, float *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::bind_out(unsigned int position, double *value)
{
  if (m_state == 0)
  {
    m_cda.rc=-1; strncpy(m_cda.message,"cursor not open.\n",128); return -1;
...
int SqlStatement::execute()
{
  ...
  // 处理字符串字段为空的情况。
  for (int ii=0; ii<max_bind_in; ii++)
  {
  ...
  if (mysql_stmt_execute(m_handle) != 0)
  {
    ...
    if (m_cda.rc==1243) memcpy(&m_cda,&m_cda1,sizeof(struct CdaDef));
