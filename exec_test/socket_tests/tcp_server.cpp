...
void Father_EXIT(int sig);  // 父进程退出函数。
void Child_EXIT(int sig);   // 子进程退出函数。

int main(int argc,char *argv[])
{
  if (argc!=3)
  {
    printf("Using:./tcp_server port logfile\n"
                  "Example:./tcp_server 54321 /tmp/socket_test/tcp_server.log\n\n"); return -1;
  }

  // 关闭全部的信号和输入输出。
  // 设置信号,在shell状态下可用 "kill + 进程号" 正常终止些进程
  // 但请不要用 "kill -9 +进程号" 强行终止
  CloseIOAndSignal(); signal(SIGINT,Father_EXIT); signal(SIGTERM,Father_EXIT);

  if (logfile.Open(argv[2],"a+")==false) { printf("logfile.Open(%s) failed.\n",argv[2]); return -1; }

  // 服务端初始化。
  if (TcpServer.InitServer(strtol(argv[1], nullptr, 10))==false)
  {
  ...
  while (true)
  {
    ...
    // 子进程重新设置退出信号。
    signal(SIGINT,Child_EXIT); signal(SIGTERM,Child_EXIT);
