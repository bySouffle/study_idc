...
int main(int argc,char *argv[])
{
  ...
  while (true)
  {
    ...
    if (_tcp_put_files()==false) { logfile.Write("_tcp_put_files() failed.\n"); EXIT(-1); }
//    break;
    if (b_continue==false)
    {
...
bool xml_parse(char *xml_buffer)
{
  ...
  if (strlen(tcp_args.ip)==0) { logfile.Write("ip is null.\n"); return false; }

  GetXMLBuffer(xml_buffer,"port",&tcp_args.port);
  if ( tcp_args.port==0) { logfile.Write("port is null.\n"); return false; }

  GetXMLBuffer(xml_buffer,"proc_type",&tcp_args.proc_type);
  if ((tcp_args.proc_type!=1)&&(tcp_args.proc_type!=2)) { logfile.Write("proc_type not in (1,2).\n"); return false; }

  GetXMLBuffer(xml_buffer,"client_path",tcp_args.client_path);
  if (strlen(tcp_args.client_path)==0) { logfile.Write("client_path is null.\n"); return false; }

  GetXMLBuffer(xml_buffer,"client_path_backup",tcp_args.client_path_backup);
  if ((tcp_args.proc_type==2)&&(strlen(tcp_args.client_path_backup)==0)) { logfile.Write("client_path_backup is null.\n"); return false; }
  ...
  if (strlen(tcp_args.match_rule)==0) { logfile.Write("match_rule is null.\n"); return false; }

  GetXMLBuffer(xml_buffer,"server_path",tcp_args.server_path);
  if (strlen(tcp_args.server_path)==0) { logfile.Write("server_path is null.\n"); return false; }

  GetXMLBuffer(xml_buffer,"scan_time",&tcp_args.scan_time);
  if (tcp_args.scan_time==0) { logfile.Write("scan_time is null.\n"); return false; }
  ...
  if (tcp_args.timeout==0) { logfile.Write("timeout is null.\n"); return false; }
  if (tcp_args.timeout<50) tcp_args.timeout=50;

  GetXMLBuffer(xml_buffer,"proc_name",tcp_args.proc_name,50);
  if (strlen(tcp_args.proc_name)==0) { logfile.Write("proc_name is null.\n"); return false; }
...
bool _tcp_put_files()
{
  ...
  {
    logfile.Write("Dir.OpenDir(%s) 失败。\n",tcp_args.client_path); return false;
  }
  ...
  while (true)
  {
    ...
    {
      memset(recv_buffer,0,sizeof(recv_buffer));
      if (TcpRead(TcpClient.m_conn_fd,recv_buffer,&buff_len,-1)==false) break;
      //  TODO  解析应答
...
bool ack_message_deal_local_files(const char *recv_buffer)
{
  ...
  GetXMLBuffer(recv_buffer,"filename",filename,255);
  GetXMLBuffer(recv_buffer,"result",result,15);
  ...
  if (tcp_args.proc_type==2)
  {
    ...
    { logfile.Write("RENAME(%s,%s) failed.\n",filename,backup_filename); return false; }
  }
